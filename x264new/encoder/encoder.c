...
x264_t *x264_encoder_open( x264_param_t *param )
{
    ...
    printf("0: I_delay: %d and %d \n", h->frames.i_delay, h->param.rc.i_lookahead);
...
int *     x264_encoder_encode( x264_t *h,
                             ...
                             x264_picture_t *pic_out )
{
    int *twoResult = (int*)malloc(sizeof(int)*2);
    ...
    *pp_nal = NULL;
    ...
    if( pic_in != NULL )
    {
        ...
        if( h->frames.i_input <= h->frames.i_delay + 1 - h->i_thread_frames )
        {
            ...
            twoResult[0] = 0;
            twoResult[1] = h->i_frame;
            // printf("twoResult[0]: %d\n", twoResult[0]);
            // printf("twoResult[1]: %d\n", twoResult[1]);
            return twoResult;
            //return 0;
        }
    }
    ...
    h->i_frame++;
    // printf("inside encoder, h->i_frame: %d\n", h->i_frame);
    /* 3: The picture is analyzed in the lookahead */
    if( !h->frames.current[0] )
        x264_lookahead_get_frames( h );

    if( !h->frames.current[0] && x264_lookahead_is_empty( h ) ){
        twoResult[0] = encoder_frame_end( thread_oldest, thread_current, pp_nal, pi_nal, pic_out );
        twoResult[1] = h->i_frame;
        return twoResult;
    ...
    else if( h->param.b_sliced_threads )
    {
        ...
        if( (intptr_t)slices_write( h ) )
            return -1;
    // FILE *test = fopen("base", "w");
    // FILE *testNal = fopen("nalu", "w");
    // // fwrite(h->fdec[0].base, sizeof(uint8_t), sizeof(h->fdec[0].base) , test );
    // // fwrite(h->out.nal[0].p_payload, sizeof(uint8_t), sizeof(h->out.nal[0].p_payload) , test );
    // // fwrite(h->out.nal[1].p_payload, sizeof(uint8_t), sizeof(h->out.nal[1].p_payload) , test );
    // // fwrite(h->out.nal[2].p_payload, sizeof(uint8_t), sizeof(h->out.nal[2].p_payload) , test );
    // for (int i = 0; i < 65; i++){
    //    fprintf(test, "%2X ", h->fenc[0].base[i]);
    // }
    // for (int i = 0; i < 65; i++){
    //    fprintf(testNal, "%2X ", h->out.nal[0].p_payload[i]);
    // }
    // printf("%d and %d\n", h->out.nal[0].i_payload, sizeof(h->out.nal[0].p_payload)/sizeof(uint8_t));
