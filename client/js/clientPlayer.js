...
myPlayer.prototype.update_state_arr = function (datasize, download_dur, chunk_dur, chunk_num, br_idx){
	...
	this.state_obv[0].push(datasize*8/1000/2400); 											// data size in Mbits
	this.state_obv[1].push(download_dur/1000);												// download duration
	this.state_obv[2].push((this.buffer.buffer_length + chunk_dur)/1000/20);					// chunk time duration
	this.state_obv[5].push(Math.log(this.bitrates[br_idx]/this.bitrates[0])/3.0);														// number of chunks
	//this.state_obv[4].push(this.bitrates[br_idx]/this.bitrates[0]);							//bitrate ratio 
	this.state_obv[6].push(Math.min(this.buffer.pre_lat/1000/20,1));																// sync
	this.state_obv[7].push(this.buffer.state/2.0);
	this.state_obv[3].push(Math.max(chunk_num*this.buffer.chunk_duration - download_dur, 0.0)/1000/0.2);		// server wait				
	this.state_obv[4].push(this.buffer.freezing/1000/3.0);
