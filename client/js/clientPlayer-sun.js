...
function myPlayer(_playerconfig, server_addr, server_port, fps, target_latency, chunk_in_seg, startup){
	this.state = 0;
	this.socket;
	//this.model = this.loadModel(server_addr); // siquan 
...
myPlayer.prototype.state_init = function() {
	...
	for (var i = 0; i < S_INFO; i++) {
		state_arr[i] = new Array(S_LEN).fill(0);
	}
	return state_arr;
}
...
myPlayer.prototype.loadModel = async function(server_addr) {
	...
	this.model = await tf.loadFrozenModel(FROZEN, MODEL_URL);
...
myPlayer.prototype.update_state_arr = function (datasize, download_dur, chunk_dur, chunk_num, br_idx){
	...
	this.state_obv[0].push(datasize*8/1000000); 											// data size in Mbits
	this.state_obv[1].push(download_dur/1000);												// download duration
	this.state_obv[2].push((this.buffer.buffer_length + chunk_dur)/1000);					// chunk time duration
	this.state_obv[3].push(chunk_num);														// number of chunks
	this.state_obv[4].push(this.bitrates[br_idx]/this.bitrates[0]);							//bitrate ratio
	this.state_obv[5].push(0);																// sync
	this.state_obv[6].push((chunk_num*this.buffer.chunk_duration - download_dur)/1000);		// server wait				
	this.state_obv[7].push(this.buffer.freezing/1000);
