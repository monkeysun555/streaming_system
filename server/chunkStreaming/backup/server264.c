...
int serverEncoder(ws_list *l, ws_client *client) {
	...
	int entire_length = 0;
	uint8_t* chunk_temp = new uint8_t[MAX_CHUNK_LEN];
	...
	// while(1){
		...
		for (int i = 0; i < iNal; ++i) {
			...
			memcpy(chunk_temp + entire_length, pNals[i].p_payload, pNals[i].i_payload);
			entire_length += pNals[i].i_payload;
			// m->msg = nal_temp;
			// nal_temp = NULL;
		...
		if (uiComponent%25 == 0 && entire_length > 0){
			printf("Entered\n");
			printf("%d\n", entire_length);
			ws_message *m = message_new();
			m->len = entire_length;
			...
			if ( (status = encodeMessage(m, 0)) != CONTINUE) {
				...
				raise(SIGINT);
				// break;
				printf("Entered error encoding\n");
			}
			memset(chunk_temp, 0, MAX_CHUNK_LEN);
			printf("Entered half\n");
			ws_send(client, m);
			message_free(m);
			free(m);
			printf("Entered FINAL\n");
		}
	}
	// fclose(pFile);
	fclose(pInput);
